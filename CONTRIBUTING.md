...
Install [ffmpeg](https://ffmpeg.org/) first (`brew install ffmpeg` on macOS, `sudo apt install ffmpeg` on Debian/Ubuntu) — the music mix and video render stages shell out to it.
...
See the [README](README.md#3-environment-variables) for the full list of environment variables (KIE.ai, Cloudflare R2, and auth secrets).

## Before opening a PR

- Run `npm run lint` and `npm run format` — the project uses ESLint + Prettier (with `prettier-plugin-organize-imports`).
- Run `npm test` — Vitest, capped at 2 forks.
